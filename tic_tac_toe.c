...
// print structure and who is win function prototype
void print_structure(int table[][4], int n);
void print_winner_structure(int table[][4], int n, int m);
...
int main()
{
    ...
    while (true) {
        ...
        if (who_is_win(table, n) == 1) {
            ...
            print_winner_structure(table, n, 1);
            break;
        } else if (who_is_win(table, n) == 2) {
            printf("Player 2 is Won!\n");
            print_winner_structure(table, n, 2);
            break;
        }
    }
...
void print_winner_structure(int table[][4], int n, int m)
{
    ...
    for (int i = 1; i <= n; i++) {
        for (int j = 1; j <= n; j++) {
            if (table[i][j] == -1)
                printf(" ");
            else if (table[i][j] == 1 && m == 1)
                printf("X");
            else if (table[i][j] == 2 && m == 2) 
                printf("O");
...
// who is win function which return winner player number
int who_is_win(int table[][4], int n)
{
    // check for all row match
    for (int i = 1; i <= 3; i++) {
